...
public class Assignment9 {//start of assignment 9
	...
	public static void main(String[] args) throws IOException {//begin main method
		...
		//set up variable length thing (initially used an array of 100 but that would mess up the recursion code, so made an array list above
		// and coverting it to an array here in order to not start my code over
		
		int[] arr = arry.stream().mapToInt(Integer::intValue).toArray();
	...
	public static int computeSumOfNumbersAtIndexDivisibleBy3(int[] elements, int startIndex, int endIndex) {
		...
		if(startIndex != endIndex) {
			sum = sum + computeSumOfNumbersAtIndexDivisibleBy3(elements, startIndex+1,endIndex);// will add the return value of number at index divisible by 3
		}
		
		if(startIndex%3 == 0) {
			sum = sum + elements[startIndex];// will add this value of the index to the sum if index is divisible by 3
		}
